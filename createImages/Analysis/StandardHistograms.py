...
def getStandardHistogram(name):
    ...
    if (name == "etmiss"):          return ROOT.TH1D("etmiss",          "Missing Transverse Momentum;E_{T,Miss} [GeV];Events", 100, 0,1000)
                                                                        
    if (name == "n_jets"):          return ROOT.TH1D("n_jets",          "Number of Jets;N_{jets};Events", 10, -0.5, 9.5)
    if (name == "jet_pt"):          return ROOT.TH1D("jet_pt",          "Jet Transverse Momentum;p_{T}^{jet} [GeV];Jets", 50, 0, 500)
    ...
    if (name == "lep_pt"):          return ROOT.TH1D("lep_pt",          "Lepton Transverse Momentum;p_{T}^{lep} [GeV];Leptons", 50, 0, 500)
