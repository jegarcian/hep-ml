...
class ResNetModel(object):
    ...
    def loss(self, batch_x, batch_y=None):
        ...
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=y_predict, labels=batch_y) # TF 1
...
def _get_variable(name, shape, initializer, weight_decay=0.0, dtype='float', trainable=True):
    ...
    if weight_decay > 0:
        regularizer = tf.contrib.layers.l2_regularizer(weight_decay) # TF 1
